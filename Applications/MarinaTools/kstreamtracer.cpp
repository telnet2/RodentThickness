...
void StreamTracer::GenerateNormals(vtkPolyData* output, double* firstNormal,
                                   const char *vecName)
{
    // Useful pointers
    vtkDataSetAttributes* outputPD = output->GetPointData();
...
// than Integrate.
void StreamTracer::SimpleIntegrate(double seed[3],
                                   double lastPoint[3],
                                   double stepSize,
                                   vtkAbstractInterpolatedVelocityField* func)
{
...
int StreamTracer::FillInputPortInformation(int port, vtkInformation *info)
{
    if (port == 0)
    {
...
void StreamTracer::PrintSelf(ostream& os, vtkIndent indent)
{
...
vtkExecutive* StreamTracer::CreateDefaultExecutive()
{
